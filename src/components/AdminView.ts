...
export class AdminView {
          ...
          <form id="adminNewSceneForm" class="admin-modal-form">
            ...
            <div class="admin-form-group">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; flex-wrap: wrap; gap: 8px;">
                <label for="newSceneDialogues" style="margin: 0; font-weight: 600;">
                  Dialoglar va Subtitrlar <span style="color: #EF4444;">*</span>
                  <small style="font-weight: normal; color: #94A3B8;">(Boshlanish(s) | Tugash(s) | Inglizcha | O‘zbekcha)</small>
                </label>
                <div style="display: flex; gap: 6px; flex-wrap: wrap;">
                  <button type="button" id="adminAutoGenerateDialoguesBtn" class="admin-btn admin-btn-secondary" style="padding: 4px 10px; font-size: 0.78rem; background: rgba(59, 130, 246, 0.15); color: #60A5FA; border: 1px solid rgba(59, 130, 246, 0.3);" title="YouTube video havolasi asosida dialoglarni avto-generatsiya qilish">
                    <i class="ph ph-bold ph-sparkle"></i> ✨ YouTube dan Avto-Generatsiya
                  </button>
                  <button type="button" id="adminAddSampleDialoguesBtn" class="admin-btn admin-btn-secondary" style="padding: 4px 10px; font-size: 0.78rem; background: rgba(16, 185, 129, 0.15); color: #34D399; border: 1px solid rgba(16, 185, 129, 0.3);" title="Namuna dialog qatorlarini kiritish">
                    <i class="ph ph-bold ph-file-text"></i> 📝 Namuna Qo‘shish
                  </button>
                  <button type="button" id="adminClearDialoguesBtn" class="admin-btn admin-btn-secondary" style="padding: 4px 8px; font-size: 0.78rem;" title="Maydonni tozalash">
                    <i class="ph ph-bold ph-trash"></i>
                  </button>
                </div>
              </div>
              <textarea id="newSceneDialogues" class="admin-textarea" rows="5" placeholder="0 | 4.0 | Hello, welcome to our lesson! | Salom, darsimizga xush kelibsiz!&#10;4.1 | 8.5 | Today we will practice listening carefully. | Bugun biz diqqat bilan tinglashni mashq qilamiz.&#10;8.6 | 13.0 | Repeat after me to improve your speech. | Nutqingizni yaxshilash uchun orqamdan takrorlang."></textarea>
              <small class="admin-field-hint" style="display:block; margin-top: 4px; font-size: 0.78rem; color: #94A3B8;">
                <i class="ph ph-bold ph-info"></i> Har bir qatorda darsning replikasi va vaqti kiritiladi. YouTube videosi bo'lsa, <strong>"✨ YouTube dan Avto-Generatsiya"</strong> tugmasini bosing.
              </small>
            </div>
  ...
  private bindSceneModalEvents(): void {
    ...
    autoGenBtn?.addEventListener('click', () => {
      ...
      if (!videoId) {
        alert('Iltimos, oldin Video Havolasi maydoniga YouTube havolasini kiriting (masalan: https://www.youtube.com/watch?v=... yoki https://youtu.be/...). Agar boshqa video bo\'lsa, "📝 Namuna Qo‘shish" tugmasini bosing.');
        videoInput?.focus();
        return;
      }
      const generated = youtubeService.generateDialoguesForVideo(videoId, title, 'Qahramon');
      if (dialInput && generated.length > 0) {
        dialInput.value = generated.map((g) => `${g.startTime} | ${g.endTime} | ${g.text} | ${g.uzbekTranslation}`).join('\n');
    ...
    sampleBtn?.addEventListener('click', () => {
      ...
      if (dialInput) {
        const sampleText = [
          '0 | 4.5 | Welcome to this English lesson, listen carefully! | Ushbu darsga xush kelibsiz, diqqat bilan tinglang!',
          '4.6 | 9.5 | Pay attention to the pronunciation and rhythm. | Talaffuz va ohangga diqqat qiling.',
          '9.6 | 15.0 | Listening and typing helps you learn faster. | Tinglab yozish sizga tezroq o\'rganishga yordam beradi.',
          '15.1 | 20.0 | Great job! You are improving your English skills! | Ajoyib! Siz ingliz tili mahoratingizni oshiryapsiz!'
        ].join('\n');
        dialInput.value = dialInput.value.trim() ? dialInput.value.trim() + '\n' + sampleText : sampleText;
      }
    });
      ...
      form.addEventListener('submit', async (e) => {
        ...
        if (dialogues.length === 0) {
          ...
          if (ytId) {
            ...
            if (dialInput) {
              dialInput.value = autoGenerated.map((g) => `${g.startTime} | ${g.endTime} | ${g.text} | ${g.uzbekTranslation}`).join('\n');
            }
          }
        }

        if (dialogues.length === 0) {
          alert('Iltimos, dars uchun kamida 1 ta dialog qatorini kiriting yoki "✨ YouTube dan Avto-Generatsiya" / "📝 Namuna Qo‘shish" tugmasini bosing.');
          dialInput?.focus();
          return;
        }
